...
public:
	Encoder_CryptoAPI();
	Encoder_CryptoAPI(string path_to_session_key) : path_to_session_key(path_to_session_key) { hProv = NULL; hSessionKey = NULL; }
	~Encoder_CryptoAPI();
...
void main() {
	...
	Encoder encoder;
	string coder_str = encoder.Encrypt("УниверПервый", true);
